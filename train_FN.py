...
from models.HDN_v2.concept_network import Concept_network
from models.HDN_v2.reconstruct_net import RC_net
from models.HDN_v2.DiscriminatorNet import DiscriminatorNet
...
# reconstruct
parser.add_argument('--RC_use', default='1')
parser.add_argument('--GAN_use', default='1')
...
def main():
    ...
    con_net = Concept_network(opts = data_opts, args = args)
    rc_net = RC_net(opts = data_opts, args = args)
    discriminator_net = DiscriminatorNet(opts = data_opts, args = args)
    ...
    model.con_net = con_net
    model.reconstruction_net = rc_net
    model.discriminator = discriminator_net
    concept_net_features = model.con_net.parameters()
    rc_net_features = model.reconstruction_net.parameters()
    discriminator_net_features = model.discriminator.parameters()
    ...
    if args.optimize_MPS:
        ...
        optimizer = get_optimizer(lr, 3, options, vgg_features_var, rpn_features, hdn_features, concept_net_features, mps_features)
    # 2. resume training
    elif args.resume is not None:
        print('Loading saved model: {}'.format(os.path.join(options['logs']['dir_logs'], args.resume)))
        args.train_all = True
        optimizer = get_optimizer(lr, 2, options, vgg_features_var, rpn_features, hdn_features, concept_net_features, mps_features)
        ...
        # 4. training with pretrained RPN
            ...
            optimizer = get_optimizer(lr, 2, options, vgg_features_var, rpn_features, hdn_features, concept_net_features, rc_net_features, discriminator_net_features, mps_features)
